...
def mandelbrot_generic(width, height, center_x, center_y, zoom, rotation, numerator, denominator, max_iter, color_map, inside_cutoff=2048, anti_aliasing=2):
    lock = Lock()
    ...
    def accumulate_subpixels(offset_x, offset_y):
        ...
        lib.mandelbrot_generic(x_buf, y_buf, width*height, numerator, denominator, max_iter, inside_cutoff)
        inside = x
        outside = y
